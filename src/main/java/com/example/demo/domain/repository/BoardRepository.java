...
public interface BoardRepository extends JpaRepository<Board,Long> {
    ...
    @Query("SELECT COUNT(b) FROM bookdb.board b WHERE b.title LIKE %:keyWord%")
    Integer countWhereTitleKeyword(@Param("keyWord")String keyWord);

    @Query("SELECT COUNT(b) FROM bookdb.board b WHERE b.username LIKE %:keyWord%")
    Integer countWhereUsernameKeyword(@Param("keyWord")String keyWord);

    @Query("SELECT COUNT(b) FROM bookdb.board b WHERE b.content LIKE %:keyWord%")
